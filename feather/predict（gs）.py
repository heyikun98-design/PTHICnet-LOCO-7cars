...
def process_multiple_excel_data(file_paths):
    ...
    sum_matrix = np.zeros((16, 21))    # 记录每个位置的HIC预测值总和
    squared_sum_matrix = np.zeros((16, 21))  # 记录每个位置的HIC预测值平方和
    ...
    for file_path in file_paths:
        ...
        for _, row in df.iterrows():
            ...
            if len(parts) == 3:  # 确保坐标格式正确
                ...
                value = row['Predicted HIC Value']  # 只使用预测值
                ...
                # 计算准确率（如果存在真实值）
                if 'True HIC Value' in row:
                    pred_value = row['Predicted HIC Value']
                    true_value = row['True HIC Value']
                    accuracy = calculate_accuracy(pred_value, true_value)
    ...
    return count_matrix, mean_matrix, variance_matrix, x_range, overall_accuracy
...
def plot_statistical_heatmap(count_matrix, mean_matrix, variance_matrix, x_range, save_dir, overall_accuracy=None):
    # 绘制数据点数量分布图
    plot_single_heatmap(count_matrix, 'HIC预测值数量分布图', 
                       os.path.join(save_dir, 'count_heatmap.png'),
                       x_range=x_range,
                       is_count=True)
    
    # 绘制均值分布图
    plot_single_heatmap(mean_matrix, 'HIC Pre Mean Distribution', 
                       os.path.join(save_dir, 'pre_mean_heatmap.png'),
                       x_range=x_range,
                       overall_accuracy=overall_accuracy)
    
    # 绘制标准差分布图
    plot_single_heatmap(variance_matrix, 'HIC预测值标准差分布图',
...
def main():
    ...
    save_dir = os.path.join(folder_path, "pre")
